	...
	override func spec() {
		describe("navigating dom parser - file"){
			...
			it("should decode properties") {
				let zoo: Zoo! = LibXMLNavigatingParserDOM.createWithURL(Fixtures.Zoo.url()) >>- Zoo.decode |> ResultExt.toOptional
				...
				expect(zoo.animals[0].url).to(equal(NSURL(string: "http://en.wikipedia.org/wiki/Grumpy_Cat")))
				expect(zoo.animals[1].name).to(equal("long"))
				expect(zoo.animals[1].kind).to(equal("cat"))
				expect(zoo.animals[1].url).to(equal(NSURL(string: "http://knowyourmeme.com/memes/longcat")))
				expect(zoo.animals[2].name).to(equal("i have no idea what i'm doing"))
				expect(zoo.animals[2].kind).to(equal("dog"))
				expect(zoo.animals[2].url).to(equal(NSURL(string: "http://knowyourmeme.com/memes/i-have-no-idea-what-im-doing")))
			}
		}
		...
		describe("navigating dom parser - data"){
			...
			it("should decode properties") {
				let zoo: Zoo! = LibXMLNavigatingParserDOM.createWithData(Fixtures.Zoo.data()) >>- Zoo.decode |> ResultExt.toOptional
		...
		describe("node parser dom - file"){
			...
			it("should decode properties") {
				let zoo: Zoo! = LibXMLNodeParserDOM.createWithURL(Fixtures.Zoo.url()) >>- Zoo.decode |> ResultExt.toOptional
		...
		describe("node parser dom - data"){
			...
			it("should decode properties") {
				let zoo: Zoo! = LibXMLNodeParserDOM.createWithData(Fixtures.Zoo.data()) >>- Zoo.decode |> ResultExt.toOptional
		...
		describe("node parser reader - file"){
			...
			it("should decode properties") {
				let zoo: Zoo! = LibXMLNodeParserReader.createWithURL(Fixtures.Zoo.url()) >>- Zoo.decode |> ResultExt.toOptional
		...
		describe("node parser reader - data"){
			...
			it("should decode properties") {
				let zoo: Zoo! = LibXMLNodeParserReader.createWithData(Fixtures.Zoo.data()) >>- Zoo.decode |> ResultExt.toOptional
